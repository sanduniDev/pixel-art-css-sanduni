...
interface ImageSetupSectionProps {
  ...
  setResultDimensions: (dimensions: CanvasDimensions) => void;
  imgSetupValidation: (canvasDimensions: CanvasDimensions, pixelSize: number, frameCount: number) => void;
}

const getImageDimensions = (canvasDimensions: CanvasDimensions, pSize: number, frameAmount: number): ImageDimensions => {
...
const ImageSetupSection: React.FC<ImageSetupSectionProps> = ({
  ...
  const getPickerAction = (property: { value: number; id: string }, setProperty: (value: number) => void) => (type: string, behaviour: number) => {
