...
const LoadFromFile: React.FC<any> = (props) => {
  ...
  const showValidationMessage = (validation: any) => {
    setValidationError({
      show: false,
      title: '',
      message: '',
  ...
  const imgSetupValidation = (contextDimensions: any, size: number, frameAmount: number) => {
    ...
    if (maxWidthReached || maxHeightReached) {
      showValidationMessage({
        ...
        message: `Frame size dimensions must not exceed ${MAX_WIDTH}px width by ${MAX_HEIGHT}px height. Please increase the pixel size or divide your image into different frames.`,
  ...
  const onLoadImage = (ev: React.ChangeEvent<HTMLInputElement>) => {
